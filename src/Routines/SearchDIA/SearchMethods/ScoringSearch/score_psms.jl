...
- `max_q_value_xgboost_mbr_rescore`: Max q-value for MBR rescoring with LightGBM
- `min_PEP_neg_threshold_xgboost_rescore`: Min PEP for negative relabeling during LightGBM training
...
function score_precursor_isotope_traces(
    ...
    if psms_count >= max_psms_in_memory
        ...
        # Use a ModelConfig (AdvancedXGBoost, backed by LightGBM) for OOM path
...
function select_psm_scoring_model(
    ...
    if psms_count >= MAX_FOR_MODEL_SELECTION
        ...
        # If no model succeeded, fall back to the Simple LightGBM configuration
        if best_model_config === nothing
            @user_warn "All models failed, defaulting to Simple LightGBM (SimpleXGBoost config)"
            best_model_config = model_configs[1]  # SimpleXGBoost (LightGBM) is first
        else
...
# Supported Models
- SimpleXGBoost (LightGBM backend): Full feature set, standard hyperparameters
- AdvancedXGBoost (LightGBM backend): Full feature set, advanced hyperparameters
...
Trains the LightGBM model using configuration from ModelConfig.
"""
...
"""
    score_precursor_isotope_traces_in_memory!(best_psms::DataFrame, file_paths::Vector{String},
                                  precursors::LibraryPrecursors) -> Dict{UInt8, Vector{LightGBMModelWrapper}}
