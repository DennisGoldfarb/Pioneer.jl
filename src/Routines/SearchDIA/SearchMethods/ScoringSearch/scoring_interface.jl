...
"""
    label_bad_transfers(; add_decoys::Bool=false)

Mark transfers that appear to come from the wrong class (target/decoy).

When `add_decoys` is true, synthetic transfer decoys are generated by
shuffling the retention-time differences among transfer candidates. These
decoys are appended to the DataFrame and flagged for later removal.
"""
function label_bad_transfers(; add_decoys::Bool=false)
    op = function(df)
        ...
        if add_decoys && any(mask)
            # Ensure a flag column exists
            if :MBR_synth_decoy ∉ names(df)
                df[!, :MBR_synth_decoy] = falses(nrow(df))
            end
            ...
            decoys.MBR_prob .= decoys.MBR_prob[perm]
            decoys.target .= false
            if :decoy in names(df)
                decoys.decoy .= true
            end
            decoys.MBR_transfer_candidate .= true
            decoys.MBR_bad_transfer .= true
...
function apply_mbr_filter!(
    ...
    # 3) compute threshold using the local bad_mask plus optional synthetic decoys
    if params.calibrate_ftr_with_decoys && any(candidate_mask)
        idx = findall(candidate_mask)
        perm = randperm(length(idx))
        decoy_scores = merged_df.MBR_prob[idx[perm]]
        scores = vcat(merged_df.MBR_prob, decoy_scores)
        targets = vcat(merged_df.target, falses(length(decoy_scores)))
        decoys = vcat(bad_mask, trues(length(decoy_scores)))
        mask_all = vcat(candidate_mask, trues(length(decoy_scores)))
        τ = get_ftr_threshold(scores, targets, decoys, params.max_MBR_false_transfer_rate; mask=mask_all)
    else
        τ = get_ftr_threshold(
            merged_df.MBR_prob,
            merged_df.target,
