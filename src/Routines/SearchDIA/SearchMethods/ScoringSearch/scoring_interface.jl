...
function train_lightgbm_model_df(feature_data::DataFrame, y::AbstractVector{Bool}, params)
    feature_names = collect(Symbol, names(feature_data))
    feature_matrix = build_feature_matrix(feature_data, feature_names)
    labels = Float64.(y)
    ...
    booster = train_lightgbm_booster(param_dict, feature_matrix, labels, 100)
