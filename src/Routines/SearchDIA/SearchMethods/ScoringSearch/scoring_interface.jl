...
function train_lightgbm_model_df(feature_data::DataFrame, y::AbstractVector{Bool}, params)
    ...
    feature_matrix = lightgbm_feature_matrix(feature_data, feature_names)
    labels = Float64.(.!y)
    ...
    dataset = LightGBM.Dataset(feature_matrix; label = labels, feature_name = String.(feature_names))
    booster = LightGBM.train(booster_params, dataset; num_boost_round = 100)
    ...
    return LightGBMModelWrapper(booster, feature_names)
end
